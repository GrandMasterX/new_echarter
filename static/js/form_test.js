...
(function ($) {
    $.order = function (params) {
        ...
        function init(p) {
            ...
            objs.disocuntPopup.changePass = function (passNum) {
                ...
                table.find('select').unbind('change').change(function(){
                    if ($(this).val() == 'birthday') {
                        ...
                        $(this).closest('tr').find('input').datepicker('destroy');
                    }
                });
            }
            ...
            objs.tripDate.change(function() {

                var date = Date.parseExact($(this).val(), 'dd.MM.yyyy');
                var cfdate = Date.parseExact(objs.backTripDate.val(), 'dd.MM.yyyy');
                        ...
                        if ($(this).is('[range]')) {
                            if ($(this).attr('range') == 'min') {
                                if (!show_params[$(this).attr('rel')]) {
                                    show_params[$(this).attr('rel')] = {'min' : parseFloat($(this).val())};
                                } else {
                                    show_params[$(this).attr('rel')]['min'] = parseFloat($(this).val());
                                }
                            } else if ($(this).attr('range') == 'max') {
                                if (!show_params[$(this).attr('rel')]) {
                                    show_params[$(this).attr('rel')] = {'max': parseFloat($(this).val())};
                                } else {
                                    show_params[$(this).attr('rel')]['max'] = parseFloat($(this).val());
                                }
                            }
                            ...
                            show_params[$(this).attr('rel')] = $(this).val().split(';');
                        }
                    } else {
